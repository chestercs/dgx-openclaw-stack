...
| **MeshCore LoRa bridge (opt-in)** | Talk to a dedicated agent from a MeshCore mesh handheld (LilyGO T-Deck etc.), fully off-grid on the radio side — a USB companion radio (e.g. LilyGO T-Echo) + a pure-Python bridge holding one persistent gateway WS connection (per-turn latency ≈ LLM decode + LoRa airtime, not the ~30-50 s per-call CLI path). SMS-sized reply pipeline (chunking, `/more` paging, late-ack), pubkey allowlist for the public-radio threat model. Activate via `--profile meshcore` + `MESHCORE_ENABLED=on`. Details in [`openclaw-meshcore-bridge/README.md`](openclaw-meshcore-bridge/README.md). |
