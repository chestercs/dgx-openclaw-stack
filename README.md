...
- **Optional ultra-realistic 4K image generation.** The `openclaw-image-comfyui` MCP bridge (opt-in via `--profile image-gen`) drives the operator's existing ComfyUI install, with a v0.11.0 max-quality 4K bundle that ships five `flux-krea-*` workflow templates: FLUX.1-Krea-dev native generation + realism LoRA stack → SUPIR diffusion-restoration upscale → ~4K output. SFW iteration / max-realism / adult / SUPIR-OOM-fallback variants. The bridge ships no model weights; the recommended ~60-70 GB download is documented in [`docs/reference/image-comfyui-bridge.md`](docs/reference/image-comfyui-bridge.md).
