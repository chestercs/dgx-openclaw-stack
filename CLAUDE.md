...
- **`draftChunk` is env-knobbed** via `OPENCLAW_DISCORD_DRAFTCHUNK_MIN_CHARS`, `_MAX_CHARS`, and `_BREAK_PREFERENCE`. Each knob is independently optional — unset means the patcher leaves the field undefined and OpenClaw uses its docs defaults (200 / 800 / `"paragraph"`). Default UX with paragraph-grain edits feels chunky for short interactive replies; setting `MIN_CHARS=100` + `BREAK_PREFERENCE=line` shifts to ~2-3s line-grain edits — much closer to typing UX. The `breakPreference` enum is not exhaustively documented upstream — `"paragraph"`, `"line"`, `"sentence"` are the operator-tested values; verify any other value with the gateway log after a force-recreate. The Discord 5-edits/5s rate limit puts a soft floor at `MIN_CHARS≈80` (~13 tokens at 6 tok/s ≈ 2s/edit cadence) on a single dedicated bot account.
- **`streaming.preview.toolProgress`** stays at the docs default (`true`); not env-knobbed.
