...
const TOOL_ORCHESTRATION_CHEATSHEET_BODY =
  ...
  '- **"milyen idő lesz / időjárás / hány fok / esik-e X napon"** → NE sima `web_search` (arxiv-ot dobhat). `python_sandbox__python_exec` + **open-meteo** (ingyenes, kulcs nélkül, 7-16 nap, JSON): geokódold a várost `https://geocoding-api.open-meteo.com/v1/search?name=Budapest&count=1` → lat/lon, majd `https://api.open-meteo.com/v1/forecast?latitude=<lat>&longitude=<lon>&daily=temperature_2m_max,temperature_2m_min,weather_code,precipitation_probability_max&timezone=auto&forecast_days=7`. 🚨 SZÁMOLD KI a cél-dátumot: a prompt elején lévő MAI dátumból + a kért napnévből (pl. ma KEDD 2026-06-09 → CSÜTÖRTÖK = 2026-06-11; használd a `datetime`-ot python_exec-ben, ne fejben). A `daily.time[]` / `hourly.time[]` tömbből PONTOSAN ezt a cél-dátumot indexeld — SOHA NE a mait, ha más napot kértek. "Este" = a 18:00-22:00 `hourly` órák. A kért napnak a daily/hourly értékeit jelentsd, nem a maiakat. `weather_code` WMO: 0=tiszta, 1-3=felhős, 45-48=köd, 51-67=eső/szitálás, 71-77=hó, 80-82=zápor, 95-99=zivatar. EGY hívás = teljes hét (csütörtök ÉS szombat is benne) — ne keress naponként, és NE mondd hogy "nem találok előrejelzést". 🚨 Az időjárás-kérdés ("csütörtökön milyen idő lesz") SOHA nem cron/emlékeztető — ez ELŐREJELZÉS-lekérés MOST, az open-meteo a jövő napokat is adja; nézd meg azonnal és válaszolj.\n' +
  ...
  '    2. **Build (NE `ng serve`):** `subprocess.run(["npx","ng","build","--base-href","/<path>/"], cwd=app_dir, check=True)`. A `--base-href /<path>/` KÖTELEZŐ a subpath-hoz, különben a JS/CSS 404-el. Statikus kimenet: `dist/<app>/browser/`. 🚨 **A `src/index.html` az Angular TEMPLATE-je:** CSAK `<app-root>` + meta/title/`<base href="/<path>/">` legyen benne. SOHA ne hardcode-olj build-asset tag-et (`<script src="main-*.js">`, `<link href="styles-*.css">`) bele — az `ng build` MAGA injektálja `type="module"`-lal; ha beégeted, DUPLIKÁLT + classic-script tag lesz a kimenetben → `import` syntax error → az Angular SOHA nem bootstrap-el → üres `<app-root>`, csak a CSS-háttér látszik (ez volt a "tájkép-screenshot" bug). NE fűzz a `src/index.html` végére másik teljes HTML doksit (régi statikus landing) sem. 🚨 **Ha Tailwind utility-osztályokat írsz a HTML-be** (`flex`, `bg-*`, `text-*`): a Tailwindet KONFIGURÁLNI is kell, nem elég a class-okat beírni! (a) `tailwind.config.js` a projekt-gyökérbe `content:["./src/**/*.{html,ts}"]` + a custom színek `theme.extend.colors`-ban; (b) `src/styles.css`-be `@tailwind base; @tailwind components; @tailwind utilities;`; (c) `ng build` (Angular esbuild auto-detektálja a configot). VERIFY: a buildelt `styles-*.css` >5KB — ha ~700 byte, a Tailwind NEM fut → stílustalan, "szétesett" oldal. Custom NEM-utility osztályt (dekoratív overlay) sima CSS-ként a styles.css-be.\n' +
