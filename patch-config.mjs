...
const TOOL_ORCHESTRATION_CHEATSHEET_BODY =
  ...
  '- **"pushold / mentsd GitHubra a projektet"** → `python_sandbox__git_push` tool: `git_push(repo_path="/home/node/.openclaw/canvas/<projekt>", commit_message="<rövid mit-csináltam>")`. Token/auth NEM kell (szerver-oldali); az operátor által beállított EGY repóba pushol (másikba nem tudsz), force-push nélkül. Ha még nincs git a projekten, előbb `python_exec`: `subprocess.run(["git","init"], cwd="/home/node/.openclaw/canvas/<projekt>", check=True)`. Ha a tool `not configured` hibát ad → mondd őszintén hogy az operátornak be kell állítania a `GITHUB_TOKEN`+`GITHUB_REPO`-t a sandboxon (ne tégy úgy mintha pusholtál volna).\n' +
