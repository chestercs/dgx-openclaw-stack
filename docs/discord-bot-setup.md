...
- **Slash commands work in DM but show "You are not authorized to use this command" in guild channels.** Upstream [issue #19310](https://github.com/openclaw/openclaw/issues/19310): OpenClaw runs slash commands through a dual perm check. Patcher step 28 fixes this with open-guild defaults (`OPENCLAW_DISCORD_AUTHZ=open`, which is the default in `.env.example`). For shared / multi-tenant / public guilds, use `OPENCLAW_DISCORD_AUTHZ=allowlist` (preserve upstream conservative defaults) or `=owner-only` (lock to `OPENCLAW_DISCORD_OWNER_IDS` snowflakes). Full mechanism and trade-offs in [`docs/reference/discord-config.md`](./reference/discord-config.md#discord-slash-command-authorization-issue-19310-dual-perm-check).
...
- Patcher overrides + operator config knobs (every Discord field this stack writes, with vanilla-restore recipe): [`docs/reference/discord-config.md`](./reference/discord-config.md)
- Text-channel agent design (mention-pill, `tools.profile`, `message` tool, streaming UX): [`docs/reference/discord-text-agent.md`](./reference/discord-text-agent.md)
- Voice-channel internals, isolation, threat model, DAVE encryption: [`docs/reference/discord-voice-agent.md`](./reference/discord-voice-agent.md)
