...
- **Slash commands work in DM but show "You are not authorized to use this command" in guild channels.** Upstream issue #19310: OpenClaw runs slash commands through a dual perm check (global `channels.discord.allowFrom` + per-guild `users` array). DMs satisfy (1) implicitly via `dmPolicy: "pairing"`; guilds have no equivalent fallback, so the empty default allowlists silently block every invocation. Patcher step 28 fixes this by writing the open-guild defaults (`allowFrom: ["*"]`, `dmPolicy: "open"`, `groupPolicy: "open"`) — set `OPENCLAW_DISCORD_AUTHZ=open` (default) in `.env` and re-run `docker compose up -d --force-recreate openclaw-config-init openclaw-gateway openclaw-cli`. For shared / multi-tenant / public guilds, use `OPENCLAW_DISCORD_AUTHZ=allowlist` (preserve upstream conservative defaults) or `=owner-only` (lock to `OPENCLAW_DISCORD_OWNER_IDS` snowflakes).
